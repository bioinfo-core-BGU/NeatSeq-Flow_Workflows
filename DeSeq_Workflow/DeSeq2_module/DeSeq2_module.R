...
option_list = list(
  ...
  make_option(c("--CONTRAST"), type="character", default=NA,
              help="The DeSeq Contrast Design [Not For LRT]", metavar="character"),
...
old_create_excel_output <- function(dds,opt, contrusts, Norm_Type, output_path, dds_lrt, fixed_norm_counts, linear_fc_cutoff, post_linear_fc_cutoff, alpha_cutoff, post_alpha){
  ...
  if (Norm_Type=='VSD'){
    norm_counts = assay(varianceStabilizingTransformation(dds,blind=TRUE))
  } else if (Norm_Type=='RLOG') {
    norm_counts = assay(rlog(dds,blind=TRUE))
  } else{
    NORMALIZATION_TYPE = unlist(stringi::stri_split(str = Norm_Type ,fixed = ","))
    if (length(NORMALIZATION_TYPE)>1){
        Normalized_counts <- DESeq2::normTransform(DESeqDataSet,
                                                   f  = get(NORMALIZATION_TYPE[1]),
                                                   pc = as.numeric(NORMALIZATION_TYPE[2] ))
    } else  {
        Normalized_counts <- DESeq2::normTransform(DESeqDataSet,
                                                   f  = get(NORMALIZATION_TYPE[1]),
                                                   pc = 0 )
    }
    norm_counts <- assay(Normalized_counts)
    #norm_counts <- counts(dds,normalized=TRUE)
  }
  
  if((!missing(fixed_norm_counts)) && (!is.na(fixed_norm_counts))){
    dds_norm <- norm_counts
    norm_counts <- fixed_norm_counts[rownames(norm_counts),]
  }
  ...
  res_df = data.frame(gene = rownames(norm_counts),
                      norm_counts)
  # Save header info:
  res_df_compar_head = c("Transcript",rep("",dim(col_data)[1]))  
  ...
  for (CONTRAST in contrusts) {
    ...
    res_df <-
      results(dds,
              ...
              alpha    = alpha_cutoff,
              lfcThreshold = log2(linear_fc_cutoff) ) %>% 
      as.data.frame %>% 
      as_tibble %>% 
      ...
      dplyr::select(linearFC,pvalue,padj,pass,manual_cutoffs) %>%
      as.data.frame %>% 
      cbind(res_df,.)
  ...
  if(! missing(dds_lrt)){
    ...
    colnames(res_df)[colnames(res_df)=="linearFC"] <- paste0("linearFC.",print_name)
    colnames(res_df)[colnames(res_df)=="pvalue"] <- paste0("pvalue.",print_name)
    colnames(res_df)[colnames(res_df)=="padj"] <- paste0("padj.",print_name)
    colnames(res_df)[colnames(res_df)=="pass"] <- paste0("pass.",print_name)
    colnames(res_df)[colnames(res_df)=="manual_cutoffs"] <- paste0("manual_cutoffs.",print_name)
    
    res_df_compar_head <- c(res_df_compar_head, rep(print_name, res_dim2))
    # Add width of group to res_df_grouping
    res_df_grouping <- c(res_df_grouping,res_dim2)
  }
...
create_excel_output <- function(dds,opt, contrusts, Norm_Type, output_path, dds_lrt, fixed_norm_counts, linear_fc_cutoff, post_linear_fc_cutoff, alpha_cutoff, post_alpha,use_only_sig_gene=F,Annotation=NULL,Sig_Gene = c()){
  ...
  res_df = data.frame(gene = rownames(norm_counts),
                      norm_counts)
  # Save header info:
  res_df_compar_head = c("Transcript",rep("Normalized Counts",dim(col_data)[1]))  
  # Save widths of data groups:
  res_df_grouping <- c(1,dim(norm_counts)[2])
...
if (opt$only_clustering==FALSE){
  ...
  if (opt$COUNT_SOURCE=='RSEM'){
    ...
    countData          = countData[!( rownames(countData) %in% zero_len ),rownames(colData)]
    ...
    DESeqDataSet_base <- DESeqDataSetFromMatrix(countData = countData[,rownames(colData)],
                                                colData = colData,
                                                design = as.formula(opt$DESIGN))
  }
  ...
  if (is.na(opt$LRT)){
    ...
    DESeqDataSet_redued <- DESeq(DESeqDataSet_base, test="LRT",
    ...
    create_excel_output(DESeqDataSet_base,
                        ...
                        Sig_Gene = sig.genes,
                        Annotation = Annotation)
    genral_test_name <- "LRT"
    org_test_count <- test_count
    test_count=c('LRT',test_count)
  }
...
if (!is.na(DESeqDataSet)){
  ...
  }else if (opt$NORMALIZATION_TYPE=='VSD') {
      Normalized_counts <- varianceStabilizingTransformation(DESeqDataSet, blind=opt$BLIND)
  }else if (opt$NORMALIZATION_TYPE=='NORMAL') {
      Normalized_counts <- DESeq2::normTransform(DESeqDataSet,
                                                     f  = abs,
                                                     pc = 0 )
  }else{
      NORMALIZATION_TYPE = unlist(stringi::stri_split(str = opt$NORMALIZATION_TYPE ,fixed = ","))
      if (length(NORMALIZATION_TYPE)>1){
          Normalized_counts <- DESeq2::normTransform(DESeqDataSet,
                                                     f  = get(NORMALIZATION_TYPE[1]),
                                                     pc = as.numeric(NORMALIZATION_TYPE[2] ))
  ...
  if (!is.na(opt$removeBatchEffect)){
    ...
    if (opt$only_clustering==FALSE){
        if (is.na(opt$LRT)){
          ...
          DESeqDataSet_redued <- DESeq(DESeqDataSet, test="LRT",
                                       reduced = as.formula(opt$LRT),
                                       parallel=F)
          genral_test_name <- "LRT"
...
for (test2do in test_count){
  ...
  if (Rmarkdown_flag){
    ...
    if (test2do=='LRT'){
      opt$outDir=file.path(base_out_dir,'LRT')
